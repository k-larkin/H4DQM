...
int Waveform::check_limits(const int& x1, const int& x2) const
{
  int return_value = x2;
  if (x2>(_samples.size()-1))
    return_value = _samples.size()-2;
  return return_value;
}
  ...
  if (imax>-1)
      ...
      for (unsigned int i(0);i<nSamplesAroundMax;++i)
	{
	  if ( (imax-(nSamplesAroundMax-1)/2+i)>=x1 && (imax-(nSamplesAroundMax-1)/2+i)<=x2)
	    ...
	    {
	      if (WARNING_ERROR)
		std::cout << "WARNING::Waveform::max_amplitude::maximum found too close to gate edges. Increase gate width" << endl;
	    }
	}
      if (nSamples>3)
	{
