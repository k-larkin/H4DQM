...
void plotterTools::plotVFEAmplitude (TH1F * histo0, TH1F * histo1, TH1F * histo2, TH1F * histo3, TH1F * histo4)
{
  TString hname = TString("digitizer_allCh_max_amplitude");
  ...
  TH1F *h0=(TH1F*)(varplots[histo0->GetName()]->GetPlot());
  TH1F *h1=(TH1F*)(varplots[histo1->GetName()]->GetPlot());
  TH1F *h2=(TH1F*)(varplots[histo2->GetName()]->GetPlot());
  TH1F *h3=(TH1F*)(varplots[histo3->GetName()]->GetPlot());
  TH1F *h4=(TH1F*)(varplots[histo4->GetName()]->GetPlot());

  int maximum = fmax(h0->GetMaximum(),h1->GetMaximum());
  maximum = fmax(maximum,h2->GetMaximum());
  maximum = fmax(maximum,h3->GetMaximum());
  maximum = fmax(maximum,h4->GetMaximum());
  maximum +=10;
  h0->GetYaxis()->SetRangeUser(0,maximum);
  h1->GetYaxis()->SetRangeUser(0,maximum);
  h2->GetYaxis()->SetRangeUser(0,maximum);
  h3->GetYaxis()->SetRangeUser(0,maximum);
  h4->GetYaxis()->SetRangeUser(0,maximum);

  h0->SetLineColor(kBlack);
  h1->SetLineColor(kBlue);
  h2->SetLineColor(kRed);
  h3->SetLineColor(kSpring);
  ...
  h0->SetTitle("allCh_max_amplitude");
  h0->GetXaxis()->SetTitle("max_amplitude");
  ...
  TLegend *l=new TLegend(0.7, 0.7, 0.89, 0.89);
    ...
    l->SetFillStyle(0);
    l->SetBorderSize(0);
    l->AddEntry(h0,"Ch_0","l");
    l->AddEntry(h1,"Ch_1","l");
    l->AddEntry(h2,"Ch_2","l");
    l->AddEntry(h3,"Ch_3","l");
    l->AddEntry(h4,"Ch_4","l");
    l->Draw();
...
void plotterTools::initDigiPlots(){
  ...
  std::set<int> groups = listElements (treeStruct_.digiBoard,  treeStruct_.nDigiSamples) ; //change classification from group to board
...
//      cout<< "Plot x-axis max entry: "<<max_entry<<endl;
  ...
  for (set<int>::iterator iGroup = groups.begin () ; 
       iGroup != groups.end () ; ++iGroup)
    {
	  ...
	  addPlot(1,Form("%s_max_amplitude",name.Data()),300,1,3000,"1D",group_,module_);
	  addPlot(1,Form("%s_charge_integrated",name.Data()),200,1,5e4,"1D",group_,module_);
...
TString plotterTools::getDigiChannelName(int group, int channel){
  ...
  if( group == 8 ) board = 3;
  if( group == 10) board = 4;
  TString name = Form("digi_ch%02d", board*5+channel) ;
...
int plotterTools::getDigiChannelX(TString name){
  ...
  if( chId ==  0 ) return 0;
  if( chId ==  1 ) return 1;
  if( chId ==  2 ) return 2;//3;
  if( chId ==  3 ) return 3;//4;
  if( chId ==  4 ) return 4;//6;
  if( chId ==  5 ) return 0;//7;
  if( chId ==  6 ) return 1;//2;
  if( chId ==  7 ) return 2;//3;
  if( chId ==  8 ) return 3;//4;
  if( chId ==  9 ) return 4;//5;
  if( chId == 10 ) return 0;
  if( chId == 11 ) return 1;//2;
  if( chId == 12 ) return 2;//3;
  if( chId == 13 ) return 3;//4;
  if( chId == 14 ) return 4;//5;
  if( chId == 15 ) return 0;//7;
  if( chId == 16 ) return 1;//0;
  if( chId == 17 ) return 2;
  if( chId == 18 ) return 3;
  if( chId == 19 ) return 4;
  if( chId == 20 ) return 0;//5;
  if( chId == 21 ) return 1;//7;
  if( chId == 22 ) return 2;
  if( chId == 23 ) return 3;
  if( chId == 24 ) return 4;
  if( chId == 25 ) return 5;
...
int plotterTools::getDigiChannelY(TString name){
  ...
  std::string chStr = nameStr.substr(pos+2,2);
  int chId = atoi(chStr.c_str());
  if( chId ==  0 ) return 0;//7;
  if( chId ==  1 ) return 0;//7;
  if( chId ==  2 ) return 0;//7;
  if( chId ==  3 ) return 0;//7;
  if( chId ==  4 ) return 0;//7;
  if( chId ==  5 ) return 1;//7;
  if( chId ==  6 ) return 1;//5;
  if( chId ==  7 ) return 1;//5;
  if( chId ==  8 ) return 1;//5;
  if( chId ==  9 ) return 1;//5;
  if( chId == 10 ) return 2;//4;
  if( chId == 11 ) return 2;//4;
  if( chId == 12 ) return 2;//4;
  if( chId == 13 ) return 2;//4;
  if( chId == 14 ) return 2;//4;
  if( chId == 15 ) return 3;//4;
  if( chId == 16 ) return 3;
  if( chId == 17 ) return 3;
  if( chId == 18 ) return 3;
  if( chId == 19 ) return 3;
  if( chId == 20 ) return 4;//3;
  if( chId == 21 ) return 4;//3;
  if( chId == 22 ) return 4;//2;
  if( chId == 23 ) return 4;//2;
  if( chId == 24 ) return 4;//2;
  if( chId == 25 ) return 2;
  if( chId == 26 ) return 0;
  if( chId == 27 ) return 0;
  if( chId == 28 ) return 0;
	...
	for (uint iSample = 0 ; iSample < treeStruct_.nDigiSamples ; ++iSample)
	  {
	    ...
	    TString thisname = getDigiChannelName(treeStruct_.digiBoard[iSample],treeStruct_.digiChannel[iSample]); //changing input from group to board to account for VFE adapter channel labelling changes
...
//if (treeStruct_.digiFrequency[iSample]!=0) cout<<"digifrequency: "<<treeStruct_.digiFrequency[iSample]<<endl;
	...
	for (std::map<TString,varPlot<float>*>::iterator it=varplots.begin();it!=varplots.end();++it)
	  {
	    ...
	    int wave_amplitude_limit=it->second->waveform->check_limits(0,900); //caps the 900 sample limit for wave_amplitude and charge_integrated to the number of samples taken
	    ...
	    wave_max=it->second->waveform->max_amplitude(wave_amplitude_limit*.1, wave_amplitude_limit*.9, 5); //find max amplitude between 50 and 900 samples

	    if (wave_max.max_amplitude<20)
	      {
		//try not inverting if signal is positive
		it->second->waveform->rescale(-1); 
		Waveform::max_amplitude_informations wave_max_inv=it->second->waveform->max_amplitude(wave_amplitude_limit*.1, wave_amplitude_limit*.9, 5); //find max amplitude between 50 and 900 samples
	    ...
	    varplots[Form("%s_charge_integrated",thisname.Data())]->Fill(it->second->waveform->charge_integrated(wave_amplitude_limit*.1,wave_amplitude_limit*.9),1.); // pedestal already subtracted
	    ...
	    varplots[Form("%s_charge_integrated_vs_hodoX1",thisname.Data())]->Fill(x1,it->second->waveform->charge_integrated(0,wave_amplitude_limit)); // pedestal already subtracted
	    varplots[Form("%s_charge_integrated_vs_hodoY1",thisname.Data())]->Fill(y1,it->second->waveform->charge_integrated(0,wave_amplitude_limit)); // pedestal already subtracted
	    varplots[Form("%s_charge_integrated_vs_hodoX2",thisname.Data())]->Fill(x2,it->second->waveform->charge_integrated(0,wave_amplitude_limit)); // pedestal already subtracted
	    varplots[Form("%s_charge_integrated_vs_hodoY2",thisname.Data())]->Fill(y2,it->second->waveform->charge_integrated(0,wave_amplitude_limit)); // pedestal already subtracted

	    varplots[Form("%s_charge_integrated_vs_TDCrecoX",thisname.Data())]->Fill(tdc_recox,it->second->waveform->charge_integrated(0,wave_amplitude_limit)); // pedestal already subtracted
	    varplots[Form("%s_charge_integrated_vs_TDCrecoY",thisname.Data())]->Fill(tdc_recoy,it->second->waveform->charge_integrated(0,wave_amplitude_limit)); // pedestal already subtracted
	    
	    int x = getDigiChannelX(it->second->name);
	    int y = getDigiChannelY(it->second->name);
	    varplots["allCh_charge_integrated_map"]->Fill2D(x,y,it->second->waveform->charge_integrated(0,wave_amplitude_limit)); // pedestal already subtracted
	    if (wave_max.max_amplitude>0 and wave_max.max_amplitude < 3000) //cutting outliers
...
//for TGraph
TGraph * plotterTools::addPlot(bool doPlot, TString name,int nPoints,TString type, TString group, TString module, bool vetoFill)
{
...
void plotterTools::plotHistos(){
  ...
  int varplot_num = 0;
  vector <TObject*> plotMultiInputs;
  for (std::map<TString,varPlot<float>*>::const_iterator out=varplots.begin();out!=varplots.end();++out){
    if (varplot_num==11 or varplot_num==25 or varplot_num==39 or varplot_num==53 or varplot_num==67){
    plotMultiInputs.push_back(out->second->GetPlot());
    }
    ++varplot_num;
  ...
  //if (plotMultiInputs.size()==5){ plotVFEAmplitude ((TH1F*)plotMultiInputs[0], (TH1F*)plotMultiInputs[1], (TH1F*)plotMultiInputs[2], (TH1F*)plotMultiInputs[3], (TH1F*)plotMultiInputs[4]); }
  else if (VERBOSE) cout<<"WARNING:: Max Amplitude plots not recorded"<<endl;
}
